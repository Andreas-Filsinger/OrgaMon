...
begin
  Result := False;
  for I := 0 to FTagList.Count - 1 do
    if TExifTag(FTagList{$ifdef XE3+}.List{$endif}[I]).ID >= ID then
    begin
      if TExifTag(FTagList{$ifdef XE3+}.List{$endif}[I]).ID = ID then
      begin
        TagIndex := I;
...
begin
  ...
  try
    for I := FTagList.Count - 1 downto 0 do
    begin
      Tag := TExifTag(FTagList{$ifdef XE3+}.List{$endif}[I]);
      for ID in IDs do
        if ID = Tag.ID then
        begin
          DoDelete(I, True);
...
begin
  if ChangeType = tcID then
    for I := FTagList.Count - 1 downto 0 do
      if Tag.ID > TExifTag(FTagList{$ifdef XE3+}.List{$endif}[I]).ID then
      begin
