...
begin
  if integer(Data1)>integer(Data2) then Result:=-1
  else if integer(Data1)<integer(Data2) then Result:=1
  else Result:=0;
end;
...
Val: Int64;
 ...
 begin
  ...
  begin
    ...
    b:=Prefix+HexStr(Integer(ANode.Data),8)+'    '
        +'  Self='+HexStr(Integer(ANode),8)
        +'  Parent='+HexStr(Integer(ANode.Parent),8)
...
begin
  ...
  if Count>0 then begin
    OldCount:=Count;
    SetLength(List,SizeOf(Pointer)*OldCount);
    try
