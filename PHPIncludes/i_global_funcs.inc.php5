...
{ $rev = "0.000";
  ...
  { if ($infohtml == "") { $infohtml = __PROJECTNAME . "_Info.html"; }
...
{ $tmp = array();
  ...
  if (!$reverse) asort($tmp); 
  else arsort($tmp); 
  $indexs = array_keys($tmp);
  unset($tmp);
  $result = array();
  foreach($indexs as $index) $result[$index] = $array[$index];
  $array = $result;
}
...
{ if (file_exists($txt_file))
  { $result = rtrim_char(file_get_contents($txt_file),"\r\n");
    $result = ($not_html2txt) ? $result : htmlentities($result);  
    $result = ($not_nl2br) ? $result : nl2br($result); 
  }
  else
...
{ return (file_exists($txt_file)) ? htmlentities(rtrim_char(file_get_contents($txt_file),"\r\n")) : 
...
{ switch(true)
  { case ($number >  0): { $sign =  1; break; }
    case ($number <  0): { $sign = -1; break; }
	case ($number == 0): { $sign =  0; break; }
  }
  return $sign;
}
...
{ $path = trim($path);
  ...
  switch(true)
  { case(substr($path,0,1) == $slash): //absoluter Linux-Pfad -> führende Punkte unterdrücken
    { $leading_dots = false;
	  break;
	}
	case(strpos($path,":") !== false):  //absoluter Windows-Pfad: mit Laufwerksangabe (:) -> führende Punkte unterdrücken
	{ $leading_dots = false;
	  break;
	}
  }
  ...
  { if (substr($path,0,1) != $slash) //Pfad beginnt nicht mit Slash
    {  if ($leading_dots) //führende Punkte: JA
	   ...
	   else //führende Punkte: NEIN
	   { //nichts tun, Pfad ist eher relativ
	   }
	}
...
//DATEIEN
function file_extension($filename, $case = 1, $dot = 0)
