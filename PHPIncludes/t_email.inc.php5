...
{ private $headers = array();
  ...
  { if (isset($this->headers[$type])) 
	...
	else $this->headers[$type] = self::serializeEmails($value);
  }
  ...
  { // Absendedatum im Format Weekday, DD-Mon-YY HH:MM:SS TIMEZONE 
    if ($value == 0) $date = date("D, d M Y H:i:s") . " GMT";
	else $date = $value;
  ...
  { //$styles = html_get_field($this->html, "", "STYLE");
    //var_dump($styles);
	$styles = array();
	return $styles;
  }
  ...
  { switch(true)
	...
	if ($type     != "") { $mime_header['Content-Type'] .= ";" . $this->crlf . "\ttype=\"" . $type . "\""; }
    if ($boundary != "") { $mime_header['Content-Type'] .= ";" . $this->crlf . "\tboundary=\"" . $boundary . "\""; }
  ...
  { $body = wordwrap($this->text,75,$this->crlf) . $this->crlf(2);
	...
	{ if ($this->html_is_set)
	  ...
	  for($i=0; $i < count($this->images);$i++) { $body = str_replace("~ImagePart".$i."~",$imagepart[$i],$body); }
	  for($i=0; $i < count($this->attachment);$i++) { $body = str_replace("~AttachedPart".$i."~",$attachedpart[$i],$body); }
	}
    return $body;
  }  
  ...
  { if (is_string($emails)) $emails = preg_split("/(\r*\n+[ ]*\t*)*(,|;){1}(\r*\n+[ ]*\t*)*/",$emails);
    var_dump($emails);
    ...
    { $length = strlen($line);
	  ...
	  { $c = $line[$i];
	    ...
	    { case (($dec == 32) OR ($dec == 9)) :
	      { if ($i == ($length)-1) $c = $escape . strtoupper(sprintf("%02s",dechex($dec))); 
		    break; 
		  }
  ...
  { $j = 0; 
    ...
    { $tags[$i] = trim($tags[$i]);
	  ...
	  { $a = strpos(strtoupper($tags[$i]),strtoupper($attribute),0);
	    ...
	    { $a = strpos($tags[$i],"=\"",$a);
	      $o = strpos($tags[$i],"\" ",$a);
	      if ($o === false) $value = substr($tags[$i],$a);
		  else $value = substr($tags[$i],$a+1,$o-$a);
  ...
  { switch(file_extension($filename,true,true))
    { // TEXT
      case(".ASC")  : $ctt = "text/plain"; break;
	  case(".TXT")  : $ctt = "text/plain"; break;
	  case(".C")    : $ctt = "text/plain"; break;
	  case(".CC")   : $ctt = "text/plain"; break;
	  case(".H")    : $ctt = "text/plain"; break;
	  case(".HH")   : $ctt = "text/plain"; break;
	  case(".CPP")  : $ctt = "text/plain"; break;
	  case(".HPP")  : $ctt = "text/plain"; break;
	  case(".PHP")  : $ctt = "text/plain"; break;
	  case(".HTML") : $ctt = "text/html"; break;
	  case(".HTM")  : $ctt = "text/html"; break;
	  case(".CSS")  : $ctt = "text/css"; break;
	  // APPLICATION
	  case(".XLS")  : $ctt = "application/excel"; break;
	  case(".CSV")  : $ctt = "application/excel"; break;
	  case(".DOC")  : $ctt = "application/msword"; break;
	  case(".DOT")  : $ctt = "application/msword"; break;
	  case(".WRD")  : $ctt = "application/msword"; break;
	  case(".PDF")  : $ctt = "application/pdf"; break;
	  case(".PGP")  : $ctt = "application/pgp"; break;
	  case(".AI")   : $ctt = "application/postscript"; break;
	  case(".EPS")  : $ctt = "application/postscript"; break;
	  case(".PS")   : $ctt = "application/postscript"; break;
	  case(".RTF")  : $ctt = "application/rtf"; break;
	  case(".GZ")   : $ctt = "application/x-gzip"; break;
	  case(".TAR")  : $ctt = "application/x-tar"; break;
	  case(".ZIP")  : $ctt = "application/zip"; break;
	  // IMAGE
	  case(".GIF")  : $ctt = "image/gif"; break;
	  case(".PNG")  : $ctt = "image/png"; break;
	  case(".JPEG") : $ctt = "image/jpeg"; break;
	  case(".JPG")  : $ctt = "image/jpeg"; break;
	  case(".JPE")  : $ctt = "image/jpeg"; break;
	  case(".TIFF") : $ctt = "image/tiff"; break;
	  case(".TIF")  : $ctt = "image/tiff"; break;
	  // AUDIO
	  case(".AU")   : $ctt = "audio/basic"; break;
	  case(".SND")  : $ctt = "audio/basic"; break;
	  case(".MIDI") : $ctt = "audio/midi"; break;
	  case(".MID")  : $ctt = "audio/midi"; break;
	  case(".KAR")  : $ctt = "audio/midi"; break;
	  case(".MPGA") : $ctt = "audio/mpeg"; break;
	  case(".MP2")  : $ctt = "audio/mpeg"; break;
	  case(".MP3")  : $ctt = "audio/mpeg"; break;
	  case(".RA")   : $ctt = "audio/x-pn-realaudio"; break;
	  case(".RAM")  : $ctt = "audio/x-pn-realaudio"; break;
	  case(".RPM")  : $ctt = "audio/x-pn-realaudio-plugin"; break;
	  case(".WAV")  : $ctt = "audio/x-wav"; break;
		
	  default : $ctt = "application/octet-stream"; break;
	}
	return $ctt;
  }
