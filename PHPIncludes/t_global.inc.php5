...
{ public $name = "";
  ...
  { $this->registered = false;
	...
	{ global $$_VAR;
	  ...
	  //FILTER
	  foreach($this->filtering as $filter)
	  { //$_VALUE = call_user_method("filter_".$filter, $this, $_VALUE);  //TS//03.08.2007//call_user_method is deprecated
	  ...
	  //VALIDATION
	  foreach($this->validating as $validator => $error_message)
	  { // $result = call_user_method("validate_".$validator, $this, $_VALUE); //TS//03.08.2007//call_user_method is deprecated
      ...
      switch($this->type) 
	  { case(T_VARIABLE_TYPE_STRING) : { $_VALUE = strval($_VALUE);   break; }
	    case(T_VARIABLE_TYPE_INTEGER): { $_VALUE = intval($_VALUE);   break; }
	    case(T_VARIABLE_TYPE_FLOAT)  : { $_VALUE = floatval($_VALUE); break; }
		case(T_VARIABLE_TYPE_ARRAY)  : {                              break; }
		default                      : { $_VALUE = strval($_VALUE);   break; }
	  }
