...
define("ERRORTITLE","<b>Es sind Fehler aufgetreten:</b>");
...
{ static private $instance = NULL;
  ...
  private function __construct($title = ERRORTITLE, $color = "#000000", $icon = false, $template = "")
  { $this->title = $title;
  ...
  static public function create($title = ERRORTITLE, $color = "#000000", $icon = false, $template = "")
  ...
  { if ($this->error AND $this->template != "") 
    { $html = $this->template;
	  ...
	  if ($this->icon != false) { $html = str_replace("~ICON~",image_tag($this->icon,"error"),$html); }
  ...
  { if ($this->error) { return implode($separator,$this->error_msg); }
    else return false;
  }
