...
const
  ...
  Content_Mode_xls2ml = 13; // xls+Vorlage.ml -> xml/html
...
function rweformat(ART, ZaehlerNummerAlt, ZaehlerNummerNeu: string): string;
var
  PreMinus: string;
  PostMinus: string;
begin
...
procedure xls2xml(InFName: string; sBericht: TStringList);
const
...
var
  ...
  STATUS: integer;
  ZAEHLWERKE_AUS_PROTOKOLL: boolean;
  ...
  begin
    result := '(' + inttostr(ZaehlwerkeAusbauSoll.count) + ':' +
      inttostr(ZaehlwerkeEinbauSoll.count) + ')';
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        (*
          ev. könnte auch ein String vorliegen, dann müsste man diesen alten
          Code reaktivieren ...
          ...
          while (pos('  ', s) > 0) do
          ersetze('  ', ' ', s);
          d := mkDateTime(date2long(nextp(s, ' ', 0)),
  ...
  procedure OneFoundClassic(r: integer);
  ...
  var
    ...
    begin
      ...
      repeat
        ...
        // Prüfen, ob dieses Zählwerk vorhanden ist!
        Index_ZW := Zaehlwerke.indexof(odis);
        if (Index_ZW = -1) then
        begin
          failBecause(
            { } 'Zählerstand aus ' + Spalte + ' kann nicht gemeldet werden. ' +
            { } 'Zählwerk ' + odis + ' ist nicht beauftragt!');
          break;
        end;
        ...
        if (Stand = '') then
        begin
          if (STATUS <> cSTATUS_Unmoeglich) then
            failBecause('Zählerstand in "' + Spalte + '" für "' + odis +
        ...
        begin
          ...
          // Summe erhöhen, nur gültig wenn alle Zählwerke>=0
          if (Zaehlwerksumme >= 0.0) then
          begin
            if (StandAsDouble >= 0.0) then
              Zaehlwerksumme := Zaehlwerksumme + StandAsDouble
            else
              Zaehlwerksumme := StandAsDouble;
          end;

          // Ausgabe
          if (StandAsDouble >= 0.0) then
          begin
            push('COUNTER ' + 'edis_key="' + odis + '"');
            speak('<!-- source="' + Spalte + '" -->');
            single('VALUE ' + 'quantity="' + Stand + '"');
            pop;
          end;
        end;

        // verbrenne das Zählwerk
        Zaehlwerke.delete(Index_ZW);
    ...
    begin
      ...
      if (Zaehlwerksumme > 0.0) then
      begin
        push('COUNTER ' + 'edis_key="' + Zaehlwerke[Index_Summe] + '"');
        speak('<!-- source="calculated value" -->');
        single('VALUE ' + 'quantity="' + inttostr(round(Zaehlwerksumme)) + '"');
        pop;
      end;
  ...
  begin
    ...
    begin
      ...
      push('ARTICLE ' +
        ...
        { } 'defect="FALSE" ' +
        { } 'no_further_use="FALSE" ' +
        { } 'multiplication_constant="1"');

      Zaehlwerksumme := 0.0;

      // Stromrichtung: E I N S P E I S E R
      Index_ET_HT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_1);
      if (Index_ET_HT = -1) then
        Index_ET_HT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_1_B);
      if (Index_ET_HT <> -1) then
      begin

        // melde Energierichtung Einspeisung!
        Index_NT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2);
        if (Index_NT = -1) then
          Index_NT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2_B);
        if (Index_NT <> -1) then
        begin

          // 2 Tarif
          MeldeZaehlwerk(r, 'EB',
            { } ZaehlwerkeAusbauSoll[Index_ET_HT],
            { } ZaehlwerkeAusbauSoll);

          // Noch mal suchen, da "MeldeZaehlwerk" die Liste verkleinert!
          Index_NT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2);
          if (Index_NT = -1) then
            Index_NT := ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2_B);
          MeldeZaehlwerk(r, 'EA',
            { } ZaehlwerkeAusbauSoll[Index_NT],
            { } ZaehlwerkeAusbauSoll);
      ...
      // Soll noch ein Summenlaufwerk gemeldet werden?
      Index_Summe := ZaehlwerkeAusbauSoll.indexof(ODIS_Summe_Einspeise);
      if Index_Summe = -1 then
        Index_Summe := ZaehlwerkeAusbauSoll.indexof(ODIS_Summe_Einspeise_B);
      if (Index_Summe <> -1) then
        MeldeSumme(Index_Summe, ZaehlwerkeAusbauSoll);

      Zaehlwerksumme := 0.0;

      // Stromrichtung: V E R B R A U C H E R
      if (ZaehlwerkeAusbauSoll.count > 1) then
      begin
      ...
      // Summe melden
      Index_Summe := ZaehlwerkeAusbauSoll.indexof(ODIS_Summe_Verbrauch);
      if Index_Summe = -1 then
        Index_Summe := ZaehlwerkeAusbauSoll.indexof(ODIS_Summe_Verbrauch_B);
      if (Index_Summe <> -1) then
        MeldeSumme(Index_Summe, ZaehlwerkeAusbauSoll);
      ...
      (*
        ...
        if (ZaehlwerkeAusbauSoll.count > 0) then
        begin
        MeldeZaehlwerk(r, 'NB', odis(3, ZaehlwerkeAusbauSoll),
        ZaehlwerkeAusbauSoll);
        end;
      *)
      pop;
    ...
    begin
      ...
      begin
        ...
        begin
          MehrInfos := MehrInfos + ' ET=' + x(r, 'ZaehlerStandAlt');
        end;
      end;

    end;

    for n := 0 to pred(ZaehlwerkeAusbauSoll.count) do
      failBecause('Zählwerk-Ausbau "' + ZaehlwerkeAusbauSoll[n] +
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Index_ET_HT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_1);
          if (Index_ET_HT = -1) then
            Index_ET_HT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_1_B);

          if (Index_ET_HT <> -1) then
          begin

            // melde Energierichtung Einspeisung!
            Index_NT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_2);
            if (Index_NT = -1) then
              Index_NT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_2_B);
            if (Index_NT <> -1) then
            begin
              // 2 Tarif

              MeldeZaehlwerk(r, 'EO',
                { } ZaehlwerkeEinbauSoll[Index_ET_HT],
                { } ZaehlwerkeEinbauSoll);

              // noch mal den Index suchen, da Liste inzwischen verkürzt
              Index_NT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_2);
              if (Index_NT = -1) then
                Index_NT := ZaehlwerkeEinbauSoll.indexof(ODIS_Einspeise_2_B);
              MeldeZaehlwerk(r, 'EN',
                { } ZaehlwerkeEinbauSoll[Index_NT],
                { } ZaehlwerkeEinbauSoll);
          ...
          // Soll noch ein Summenlaufwerk gemeldet werden?
          Index_Summe := ZaehlwerkeEinbauSoll.indexof(ODIS_Summe_Einspeise);
          if Index_Summe = -1 then
            Index_Summe := ZaehlwerkeEinbauSoll.indexof(ODIS_Summe_Einspeise_B);
          if (Index_Summe <> -1) then
            MeldeSumme(Index_Summe, ZaehlwerkeEinbauSoll);
          ...
          Index_Summe := ZaehlwerkeEinbauSoll.indexof(ODIS_Summe_Verbrauch);
          if Index_Summe = -1 then
            Index_Summe := ZaehlwerkeEinbauSoll.indexof(ODIS_Summe_Verbrauch_B);
          if (Index_Summe <> -1) then
            MeldeSumme(Index_Summe, ZaehlwerkeEinbauSoll);

        end;

        // Übrig gebliebene Zählwerke
        for n := 0 to pred(ZaehlwerkeEinbauSoll.count) do
          failBecause('Zählwerk-Einbau "' + ZaehlwerkeEinbauSoll[n] +
            '" konnte nicht zugeordnet werden!');
  ...
  procedure OneFoundProtocol(r: integer);
  ...
  var
    ...
    procedure MeldeZaehlwerk(r, z: integer);
    var
      Stand: string;
      StandAsDouble: double;
      SpalteName: string;
...
begin
  ...
  begin
    ...
    add('<!DOCTYPE FILE SYSTEM "ArbeitsschritteImport.dtd" []>');
    push('FILE');
    speak;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            begin
              fillAufgaben;
              if ZAEHLWERKE_AUS_PROTOKOLL then
              begin
                OneFoundProtocol(r);
              ...
              begin
                ...
                begin
                  // .count = 2
                  if (ZaehlwerkeAusbauSoll.indexof('1-1:1.8.2') <> -1) or
                    (ZaehlwerkeAusbauSoll.indexof('1-0:1.8.2') <> -1) or
                    (ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2) <> -1) or
                    (ZaehlwerkeAusbauSoll.indexof(ODIS_Einspeise_2_B) <> -1) or
          ...
          begin
            if assigned(sBericht) then
              sBericht.add('(RID=' + RID +
                ') STATUS sollte in (Unmöglich,Vorgezogen,Erfolg) sein!');
          end;
        end
        else
        begin
  ...
  begin
    ...
    for cErgebnis := 0 to pred(sResult.count) do
      ...
      begin
        ...
        if (cAuftrag = -1) then
          raise exception.create(format('Spalte "%s" im Auftrag nicht gefunden',
            [header[cErgebnis]]));
...
begin
  ...
  try
    ...
    begin
      inc(ErrorCount);
      sDiagnose.add(cERRORText + ' ' + e.message);
      sDiagnose.add(cERRORText + ' ' + conversionOutFName +
        ' ist durch andere Anwendung geöffnet?');
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //
        ZAEHLER_NUMMER := sSparte + '-' +
          shortenSerial(nextp(MySourceStrings[n], ';', c_KK20_SerialNo));
      ...
      begin
        ...
        try
          ...
          NachKommaStellen := strtointdef(nextp(MySourceStrings[n], ';',
            c_KK21_NachKommaStellen), -1);
    ...
    //
    sDiagnose.add(inttostr(Stat_ergebnisse) + '/' + inttostr(Stat_Zaehler) +
      ' Zähler verarbeitet!');
...
var
  ...
  begin
    ...
    if (KeyDuplicates > 0) then
      if assigned(sBericht) then
        sBericht.add(cWARNINGText + ' Anker - Wert "' + Key + '" kommt ' +
          inttostr(KeyDuplicates + 1) + ' mal vor!');
  ...
  begin
    ...
    try
      ...
      begin
        ...
        sDiagnose.add('WARNING: Zelle ' + inttostr(r) + ',' + inttostr(c) +
...
begin
  ...
  begin
    ...
    // Ergebnis speichern
    conversionOutFName := WorkPath + sMappings.values[cMappings_Ergebnis] +
      ExtractFileName(InFName) + '.csv';
...
begin
  ...
  begin
    ...
    begin
      ...
      Error('Datei ' + InFName +
        ' ist leer, durch eine andere Anwendung gesperrt oder keine .csv!');
    end;
    BK.Free;
    csvTABELLE.Free;
  end;
...
begin
  ...
  begin
    ...
    // RWE 1.0
    if FileExists(WorkPath + 'ArbeitsschritteImport.dtd') then
    begin
      result := Content_Mode_xls2xml;
      exit;
    end;
...
procedure xls2ml(InFName: string; sBericht: TStringList);
const
  cSTATUS_Unmoeglich = 9;
  cSTATUS_Vorgezogen = 7;
  cSTATUS_Erfolg = 4;
...
var
  ...
  // Ausgabe ist ~Zaehler_Nummer~.zip.html
  // es werden immer kleine Dateien ausgegeben
  iHTML: boolean;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // bisheriges eventuell vorhandenes pdf ist ungültig!
            FileDelete(WorkPath + ZAEHLER_NUMMER + '.zip.html.pdf');
...
procedure _xmlSchemaValidityWarningFunc(ctx: pointer;
  const msg1, msg2, msg3, msg4: wasPChar); cdecl;
var
  s: string;
begin
...
begin
  ...
  begin
    ...
    sBericht.add(cERRORText + ' ' + 'XML-Datei "' + xmlFileName +
      '" konnte nicht geöffnet werden!');
end;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            begin
              if assigned(sBericht) then
                sBericht.add('(RID=' + RID + ') ARGOS "' + inttostr(Argos) +
                  '" in XML.BLA nicht gefunden!');
            end;
          end
          else
          begin
  ...
  try
    ...
    case Mode of
      ...
      Content_Mode_xls2xml:
        begin
          sDiagnose.add('Modus: xls -> xml');
          xls2xml(InFName, sBericht);
        end;
