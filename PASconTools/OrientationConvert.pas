...
var
 RecurseLevel: integer;
  ...
  begin
    ...
    if (length(s)>=5) then
      for n := 0 to pred(sTable.count) do
       ...
       begin
         result := n;
         sym := ExtractSegmentBetween(copy(sTable[n],1,5),'(',')');

         if (sym='EOS') then
         begin
          sBericht.Add(fill(' ',ReCurseLevel)+' result := true;');
  ...
  begin
   ...
   begin
    inc(RecurseLevel);
    sBericht.Add(fill(' ',ReCurseLevel)+'if B then');
    sBericht.Add(fill(' ',ReCurseLevel)+'begin { '+s+'1 }');
    TableIndex := findFull(s+'1');
    if (TableIndex=-1) then
     Check(s+'1');
    sBericht.Add(fill(' ',ReCurseLevel)+'end else');
    sBericht.Add(fill(' ',ReCurseLevel)+'begin { '+s+'0 }');
    TableIndex := findFull(s+'0');
    if (TableIndex=-1) then
     Check(s+'0');
    sBericht.Add(fill(' ',ReCurseLevel)+'end;');
    dec(RecurseLevel);
   end;
  end;
...
begin
  ...
  sBericht.add('function decode: boolen;');
  sBericht.Add('{ this source code was generated by Oc Rev. '+RevToStr(Version)+' }');
  sBericht.Add('{ Oc --huff '+InFname+' }');
  sBericht.Add('{ '+dTimeStamp(FileDateTime(InFName))+' }');
  sBericht.add('begin');
  sBericht.Add(' result := false;');
  sBericht.Add(' while true do');
  sBericht.Add(' begin');
  RecurseLevel := 1;
  Check;
  sBericht.Add(' end;');
