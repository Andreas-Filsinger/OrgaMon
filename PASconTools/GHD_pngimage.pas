{Portable Network Graphics Delphi 1.565      (31 July 2006)   }
...
{
  Version 1.565
  2009-10-26   MIGRATION 1 - Delphi 2010 Support
                             (Andreas Filsinger)
...
begin
  ...
  repeat
    ...
    repeat
      ...
      {Copy data}
      pByte(Dest2)^ := pByte(Dest2)^ or
        ( ((pByte(Src)^ shr CurBit) and BitTable[Header.BitDepth])
...
begin
  ...
  repeat
    ...
    repeat
      ...
      {Copy data}
      pByte(Dest2)^ := pByte(Dest2)^ or (((pByte(Src)^ shr CurBit) and $3)
         shl (4 - (4 * Col) mod 8));
      {Move to next column}
      inc(Col, ColumnIncrement[Pass]);
      {Will read next bits}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      Dest2 := PAnsiChar(Longint(Dest) + Col div 2);
      {Copy data}
      pByte(Dest2)^ := pByte(Dest2)^ or ((((pByte(Src)^ shr CurBit) shl 2) and $F)
         shl (4 - (Col*4) mod 8));
      {Move to next column}
      inc(Col, ColumnIncrement[Pass]);
      {Will read next bits}
  ...
  with Header.BitmapInfo.bmiHeader do
    repeat
      ...
      repeat
        ...
        {Copy data}
        pByte(Dest)^ := pByte(Dest)^ or
          (((pByte(Src2)^ shr (StartBit[Header.BitDepth] - (biBitCount * Col)
...
procedure TPngObject.Stretch(NewWidth, NewHeight: integer);
begin
...
procedure TPngObject.readText(s: TStrings);
var
  n: integer;
begin
  for n := 0 to pred(Chunks.count) do
   if Chunks.item[n].Name='tEXt' then
    with Chunks.item[n] as TChunkTEXT do
     s.add(KeyWord+'='+Text);
end;
