...
{ static private $instance = NULL;
  ...
  { //echo wrap($timeout);
    ...
    if (!$fp) 
	{ $this->setError(100, $this->server . "::" . $this->port . ": " . "Socketverbindung konnte nicht geoeffnet werden");
	  trigger_error("XMLRPC(100): was unable to open socket '" . $this->server . ":" . $this->port ."'" , E_USER_WARNING);
	  return(false);
	}
	...
	while($data = @fread($fp, txmlrpc::$block)) 
	{ $payload .= $data; 
	}
