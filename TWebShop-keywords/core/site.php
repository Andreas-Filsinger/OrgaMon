...
include_once("./core/keywords.php");
...
class tsite {
    ...
    static private function getTitleSeparator($separator)
    { return ($separator != "") ? $separator : self::$title_separator;
    }

    public function appendTitle($title, $separator = "") {
        $this->title .= self::getTitleSeparator($separator) . $title;
    }
    
    public function prependTitle($title, $separator = "") {
        $this->title = $title . self::getTitleSeparator($separator) . $this->title;
    }
    ...
    public function setKeywords($keywords, $separator = ", ") {
        $this->keywords = new tkeywords($keywords, $separator);
    }
    ...
    public function appendKeywords($keywords) {
        if ($this->keywords == NULL) {
            ...
            $this->keywords->appendList($keywords);
        }         
    }

    public function getKeywords($as_array = false) {
        return ($this->keywords != NULL) ? $this->keywords->getList($as_array) : "";
    }
