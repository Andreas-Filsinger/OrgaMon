...
if ($site->isActive()) {
    ...
    $article->addOption("DEMO", (count($article->getSounds()) > 0) ? _TEMPLATE_ARTICLE_ARTICLE_OPTION_DEMO : "");
    ...
    $site->prependTitle($article->TITEL . " (" . trim($article->getComposer()) . ")");
    $site->appendKeywords(array($article->TITEL, $article->getComposer(), $article->getArranger()));
}
?>