...
type
  ...
  protected
    ...
    PageLabel: TLabel;
  public
    constructor Create(AOwner: TThumbnailPreview; APageNum: Integer);
    destructor Destroy; override;
...
begin
  if FLoadedIndex <> Index then
  begin
    FMetaFile.OnChange := nil;
    try
      FDataStream.Seek(Integer(FRecords[Index]), soFromBeginning);
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        SelectClipPath(FOffscreen.Canvas.Handle, ClipRgn);
      finally
      ...
      SelectClipPath(FOffscreen.Canvas.Handle, 0);
...
begin
  ...
  begin
    ...
    Bitmap := Graphic as TBitmap;
  BltBitmapAsDib(Canvas.Handle, Result.Left, Result.Top, W, H, Bitmap);
  if not (Graphic is TBitmap) then
    Bitmap.Free;
end;
...
begin
  ...
  with PDevMode(GlobalLock(DeviceMode))^ do
    try
      ...
      if IsCustomPaper then
      begin
        FPageExt := ConvertXY(dmPaperWidth, dmPaperLength, mmLoMetric, FUnits);
        if FOrientation = poLandscape then
          SwapValue(FPageExt.X, FPageExt.Y);
        CalculateMetafileSize;
        UpdateZoom;
      end
    finally
...
begin
  ...
  begin
    ...
    begin
      FCanvas.Pen.Width := ConvertX(FCanvas.Pen.Width, FUnits, Value);
      ScaleCanvas;
    end;
    FUnits := Value;
  end;
...
begin
  ...
  if (FPageExt.X <> Value) and (FState = psReady) then
  begin
    FPaperType := pCustom;
    FPageExt.X := Value;
    CalculateMetafileSize;
...
begin
  ...
  if (FPageExt.Y <> Value) and (FState = psReady) then
  begin
    FPaperType := pCustom;
    FPageExt.Y := Value;
    CalculateMetafileSize;
...
procedure TPrintPreview.ScaleCanvas;
var
  FontSize: Integer;
...
begin
  ...
  DevExt.X := ConvertUnits(LogExt.X,
    GetDeviceCaps(FCanvas.Handle, LOGPIXELSX), FUnits, mmPixel);
  DevExt.Y := ConvertUnits(LogExt.Y,
    GetDeviceCaps(FCanvas.Handle, LOGPIXELSY), FUnits, mmPixel);
  SetMapMode(FCanvas.Handle, MM_ANISOTROPIC);
  SetWindowExtEx(FCanvas.Handle, LogExt.X, LogExt.Y, nil);
  SetViewPortExtEx(FCanvas.Handle, DevExt.X, DevExt.Y, nil);
  SetViewportOrgEx(FCanvas.Handle,
    -GetDeviceCaps(FCanvas.Handle, PHYSICALOFFSETX),
    -GetDeviceCaps(FCanvas.Handle, PHYSICALOFFSETY), nil);
  FontSize := FCanvas.Font.Size;
  FCanvas.Font.PixelsPerInch :=
    MulDiv(GetDeviceCaps(FCanvas.Handle, LOGPIXELSY), LogExt.Y, DevExt.Y);
...
begin
  ...
  try
    MetaFile.Width := FDeviceExt.X;
    MetaFile.Height := FDeviceExt.Y;
...
procedure TPrintPreview.CreatePrinterCanvas;
begin
  FCanvas := TCanvas.Create;
  try
    FCanvas.Handle := Printer.Handle;
    ScaleCanvas;
  except
    FCanvas.Free;
    raise;
...
procedure TPrintPreview.ClosePrinterCanvas;
begin
  FCanvas.Handle := 0;
  FCanvas.Free;
  FCanvas := nil;
...
constructor TThumbnail.Create(AOwner: TThumbnailPreview; APageNum: Integer);
begin
  PageNum := APageNum;
...
var
  PageNum: Integer;
begin
  PageNum := TPaperPreview(Sender).Tag;
  PlayEnhMetaFile(Canvas.Handle, PrintPreview.Pages[PageNum].Handle, PageRect);
  if (ActiveThumb <> nil) and (ActiveThumb.PageNum = PageNum) then
  begin
    ...
    try
      ...
      begin
        ...
        for I := FThumbnails.Count - 1 downto 0 do
          if TThumbnail(FThumbnails[I]).PageNum = PrintPreview.CurrentPage then
          begin
            Thumb := TThumbnail(FThumbnails[I]);
            Break;
          end;
