...
uses
  globals, anfix32, ArtikelVerlag,
  Lager, eCommerce, CareTakerClient,
  ...
  begin
    ...
    begin
      ...
      AllData.add(FieldByName('RID').AsString + ';' +
        ...
        NoSemi(FormArtikelVerlag.ObtainVerlagFromPERSON_RID(FieldByName('VERLAG_R').AsInteger)) + ';' +
        inttostr(FieldByName('MENGE').AsInteger) + ';' +
        NoSemi(ArtikelInfo.Values['BEM']) + ';' +
        DataModuleeCommerce.e_r_LagerPlatzNameFromLAGER_R(FieldByName('LAGER_R').AsInteger) + ';' +
...
begin
  ...
  begin
    SORTIMENT := TIB_Cursor.create(self);
    with SORTIMENT do
    begin
...
begin
  ...
  SORTIMENT := TIB_Query.create(self);
  with SORTIMENT do
  begin
...
begin
  BeginHourGlass;
  label11.Caption := DataModuleeCommerce.e_r_sqls('select count(rid) from artikel where sortiment_r=' + inttostr(IB_Query1.FieldByName('RID').AsInteger));
  EndHourGlass;
end;
