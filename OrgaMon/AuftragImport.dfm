object FormAuftragImport: TFormAuftragImport
  Left = 0
  Top = 28
  Caption = 'Projekt - Import'
  ClientHeight = 666
  ClientWidth = 1017
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  OnCreate = FormCreate
  OnDeactivate = FormDeactivate
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 3
    Top = 7
    Width = 90
    Height = 13
    Caption = 'Schema (*.gzs)'
  end
  object Label4: TLabel
    Left = 480
    Top = 7
    Width = 80
    Height = 13
    Caption = 'Quelle (*.csv)'
  end
  object Image3: TImage
    Left = 953
    Top = 0
    Width = 54
    Height = 22
    Cursor = crHandPoint
    AutoSize = True
    Picture.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
    OnClick = Image3Click
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 49
    Width = 1017
    Height = 617
    ActivePage = TabSheet2
    TabOrder = 5
    object TabSheet1: TTabSheet
      Caption = 'Quelle und Zuordnungen'
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Label5: TLabel
        Left = 1
        Top = -1
        Width = 122
        Height = 13
        Caption = 'm'#246'gliche Datenfelder'
      end
      object Label6: TLabel
        Left = 276
        Top = 1
        Width = 76
        Height = 13
        Caption = 'Zuordnungen'
      end
      object Label7: TLabel
        Left = 709
        Top = 1
        Width = 102
        Height = 13
        Caption = 'verf'#252'gbare Felder'
      end
      object Label8: TLabel
        Left = 709
        Top = 295
        Width = 111
        Height = 13
        Caption = 'Beispielhafte Daten'
      end
      object Label12: TLabel
        Left = 368
        Top = -1
        Width = 307
        Height = 13
        Caption = '(Schema wurde ge'#228'ndert, speichern nicht vergessen)'
        Visible = False
      end
      object Label14: TLabel
        Left = 825
        Top = 2
        Width = 10
        Height = 13
        Caption = '()'
      end
      object ListBox1: TListBox
        Left = 0
        Top = 19
        Width = 249
        Height = 566
        ItemHeight = 13
        TabOrder = 0
        OnDblClick = ListBox1DblClick
      end
      object ListBox2: TListBox
        Left = 276
        Top = 42
        Width = 426
        Height = 437
        ItemHeight = 13
        TabOrder = 1
        OnKeyDown = ListBox2KeyDown
      end
      object ListBox3: TListBox
        Left = 708
        Top = 20
        Width = 295
        Height = 273
        ItemHeight = 13
        TabOrder = 2
        OnDblClick = ListBox3DblClick
      end
      object ListBox4: TListBox
        Left = 708
        Top = 313
        Width = 296
        Height = 272
        ItemHeight = 13
        TabOrder = 3
      end
      object Button4: TButton
        Left = 250
        Top = 234
        Width = 25
        Height = 20
        Caption = '->'
        TabOrder = 4
        OnClick = Button4Click
      end
      object Panel1: TPanel
        Left = 280
        Top = 497
        Width = 425
        Height = 89
        TabOrder = 5
        object Label9: TLabel
          Left = 7
          Top = 11
          Width = 16
          Height = 13
          Caption = '#1'
        end
        object Label10: TLabel
          Left = 7
          Top = 35
          Width = 16
          Height = 13
          Caption = '#2'
        end
        object Label11: TLabel
          Left = 7
          Top = 59
          Width = 16
          Height = 13
          Caption = '#3'
        end
        object ComboBox3: TComboBox
          Left = 33
          Top = 7
          Width = 337
          Height = 21
          TabOrder = 0
          OnChange = ComboBox3Change
        end
        object ComboBox4: TComboBox
          Left = 33
          Top = 33
          Width = 337
          Height = 21
          TabOrder = 1
          OnChange = ComboBox4Change
        end
        object ComboBox5: TComboBox
          Left = 33
          Top = 56
          Width = 337
          Height = 21
          TabOrder = 2
          OnChange = ComboBox5Change
        end
        object Button5: TButton
          Left = 376
          Top = 9
          Width = 34
          Height = 19
          Caption = '<-'
          TabOrder = 3
          OnClick = Button5Click
        end
        object Button6: TButton
          Left = 376
          Top = 33
          Width = 34
          Height = 20
          Caption = '<-'
          TabOrder = 4
          OnClick = Button6Click
        end
        object Button7: TButton
          Left = 376
          Top = 59
          Width = 34
          Height = 18
          Caption = '<-'
          TabOrder = 5
          OnClick = Button7Click
        end
      end
      object CheckBox2: TCheckBox
        Left = 838
        Top = 293
        Width = 45
        Height = 18
        Caption = 'alle'
        TabOrder = 6
        OnClick = CheckBox2Click
      end
      object Button2: TButton
        Left = 890
        Top = 293
        Width = 74
        Height = 17
        Caption = 'eindeutig?'
        TabOrder = 7
        OnClick = Button2Click
      end
      object Button13: TButton
        Left = 276
        Top = 19
        Width = 23
        Height = 21
        Hint = 'nach oben schieben'
        Caption = #241
        Font.Charset = SYMBOL_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Wingdings'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 8
        OnClick = Button13Click
      end
      object Button14: TButton
        Left = 301
        Top = 19
        Width = 21
        Height = 21
        Hint = 'nach unten schieben'
        Caption = #242
        Font.Charset = SYMBOL_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Wingdings'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 9
        OnClick = Button14Click
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Verarbeitung und Import'
      ImageIndex = 1
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object GroupBox1: TGroupBox
        Left = 552
        Top = 16
        Width = 451
        Height = 537
        TabOrder = 0
        object Label1: TLabel
          Left = 19
          Top = 152
          Width = 71
          Height = 13
          Caption = 'Import-RID:'
        end
        object Label19: TLabel
          Left = 16
          Top = 24
          Width = 200
          Height = 14
          Caption = 'M'#246'glichkeiten nach einem Import'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = [fsItalic, fsUnderline]
          ParentFont = False
        end
        object Edit2: TEdit
          Left = 98
          Top = 149
          Width = 74
          Height = 21
          TabOrder = 0
        end
        object Button1: TButton
          Left = 184
          Top = 146
          Width = 113
          Height = 25
          Caption = 'Jetzt L'#246'schen'
          TabOrder = 1
          OnClick = Button1Click
        end
        object Button11: TButton
          Left = 17
          Top = 49
          Width = 353
          Height = 30
          Caption = 'Diagnose (Info.txt) des letzten Imports'
          Enabled = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnClick = Button11Click
        end
        object Panel6: TPanel
          Left = 4
          Top = 188
          Width = 444
          Height = 1
          TabOrder = 3
        end
        object Button12: TButton
          Left = 16
          Top = 80
          Width = 354
          Height = 31
          Caption = 'Abgelehnte.csv'
          TabOrder = 4
          OnClick = Button12Click
        end
      end
      object GroupBox2: TGroupBox
        Left = 16
        Top = 16
        Width = 537
        Height = 537
        TabOrder = 1
        object Label3: TLabel
          Left = 10
          Top = 48
          Width = 71
          Height = 13
          Caption = 'in Baustelle:'
        end
        object Label13: TLabel
          Left = 183
          Top = 49
          Width = 71
          Height = 13
          Caption = 'Import-RID:'
        end
        object Label15: TLabel
          Left = 12
          Top = 91
          Width = 366
          Height = 14
          Caption = 'Vor dem Import: Vermeidung von doppelten Z'#228'hlernummern'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = [fsItalic, fsUnderline]
          ParentFont = False
        end
        object Label16: TLabel
          Left = 12
          Top = 222
          Width = 238
          Height = 14
          Caption = 'Besonderheiten bei diesem Import-Lauf'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = [fsItalic, fsUnderline]
          ParentFont = False
        end
        object Label18: TLabel
          Left = 12
          Top = 19
          Width = 252
          Height = 14
          Caption = 'In welche Baustelle soll importiert werden'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = [fsItalic, fsUnderline]
          ParentFont = False
        end
        object ProgressBar1: TProgressBar
          Left = 10
          Top = 506
          Width = 458
          Height = 16
          TabOrder = 0
        end
        object Button3: TButton
          Left = 480
          Top = 503
          Width = 50
          Height = 21
          Caption = 'Start'
          TabOrder = 1
          OnClick = Button3Click
        end
        object ComboBox6: TComboBox
          Left = 88
          Top = 46
          Width = 74
          Height = 21
          Style = csDropDownList
          TabOrder = 2
          OnDropDown = ComboBox6DropDown
        end
        object CheckBox1: TCheckBox
          Left = 12
          Top = 119
          Width = 453
          Height = 18
          Caption = 
            'doppelte Z'#228'hlernummern ablehnen und in Abgelehnt.csv speichern a' +
            'b PQ:'
          Checked = True
          State = cbChecked
          TabOrder = 3
        end
        object CheckBox3: TCheckBox
          Left = 12
          Top = 266
          Width = 389
          Height = 17
          Caption = 'nur simulieren und importierbare in Importierte.csv speichern'
          TabOrder = 4
        end
        object CheckBox5: TCheckBox
          Left = 12
          Top = 136
          Width = 261
          Height = 16
          Caption = 'Z'#228'hlernummer mit Z'#228'hler-Art erg'#228'nzen'
          TabOrder = 5
        end
        object Edit3: TEdit
          Left = 480
          Top = 117
          Width = 50
          Height = 21
          TabOrder = 6
        end
        object Panel2: TPanel
          Left = 3
          Top = 78
          Width = 530
          Height = 2
          TabOrder = 7
        end
        object Panel3: TPanel
          Left = 4
          Top = 210
          Width = 530
          Height = 1
          TabOrder = 8
        end
        object Panel4: TPanel
          Left = 4
          Top = 293
          Width = 529
          Height = 0
          TabOrder = 9
        end
        object Panel5: TPanel
          Left = 4
          Top = 491
          Width = 530
          Height = 2
          TabOrder = 10
        end
        object CheckBox12: TCheckBox
          Left = 12
          Top = 282
          Width = 401
          Height = 18
          Caption = 'leere Z'#228'hlerart nicht importieren'
          TabOrder = 11
        end
        object CheckBox13: TCheckBox
          Left = 12
          Top = 167
          Width = 502
          Height = 17
          Caption = 
            'Nur die Ziffern importieren und alle Sonderzeichen und Buchstabe' +
            'n entfernen'
          TabOrder = 12
        end
        object CheckBox14: TCheckBox
          Left = 12
          Top = 183
          Width = 361
          Height = 17
          Caption = 'Nur den letzten Ziffernblock der Z'#228'hlernummer verwenden'
          TabOrder = 13
        end
        object Edit1: TEdit
          Left = 260
          Top = 46
          Width = 103
          Height = 21
          ReadOnly = True
          TabOrder = 14
        end
        object CheckBox4: TCheckBox
          Left = 12
          Top = 250
          Width = 351
          Height = 17
          Caption = 'mehrfach ausf'#252'hren, bis Quell-Verzeichnis leer ist'
          TabOrder = 15
        end
        object CheckBox9: TCheckBox
          Left = 12
          Top = 298
          Width = 445
          Height = 17
          Caption = 'Erfolgreich importierte markieren'
          Checked = True
          State = cbChecked
          TabOrder = 16
        end
        object CheckBox10: TCheckBox
          Left = 12
          Top = 314
          Width = 469
          Height = 17
          Caption = 'bestehende Markierungen zuvor L'#246'schen'
          TabOrder = 17
        end
        object CheckBox11: TCheckBox
          Left = 12
          Top = 151
          Width = 277
          Height = 17
          Caption = 'Z'#228'hlernummer mit Materialnummer erg'#228'nzen'
          TabOrder = 18
        end
      end
    end
  end
  object Button8: TButton
    Left = 367
    Top = 24
    Width = 33
    Height = 21
    Caption = '->'
    TabOrder = 1
    OnClick = Button8Click
  end
  object Button9: TButton
    Left = 974
    Top = 27
    Width = 34
    Height = 19
    Caption = '->'
    TabOrder = 2
    OnClick = Button9Click
  end
  object Button10: TButton
    Left = 404
    Top = 24
    Width = 71
    Height = 21
    Caption = 'speichern'
    TabOrder = 4
    OnClick = Button10Click
  end
  object CheckBox6: TCheckBox
    Left = 566
    Top = 4
    Width = 52
    Height = 16
    Caption = 'MEA'
    TabOrder = 6
    OnClick = CheckBox6Click
  end
  object CheckBox7: TCheckBox
    Left = 628
    Top = 4
    Width = 141
    Height = 16
    Caption = 'Spalten'#252'berschriften'
    Checked = True
    State = cbChecked
    TabOrder = 7
    OnClick = CheckBox7Click
  end
  object CheckBox8: TCheckBox
    Left = 778
    Top = 4
    Width = 126
    Height = 16
    Caption = 'OEM Zeichensatz'
    TabOrder = 8
  end
  object Edit4: TEdit
    Left = 3
    Top = 24
    Width = 359
    Height = 21
    TabOrder = 0
  end
  object Edit5: TEdit
    Left = 480
    Top = 24
    Width = 488
    Height = 21
    TabOrder = 3
  end
  object OpenDialog1: TOpenDialog
    DefaultExt = 'GZS'
    Filter = 'GaZMa Schema (*.gzs)|*.gzs'
    Left = 168
    Top = 10
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'GZS'
    Filter = 'GaZMa Schema (*.gzs)|*.gzs'
    Left = 200
    Top = 10
  end
  object OpenDialog2: TOpenDialog
    DefaultExt = 'CSV'
    Filter = 
      'CSV (Trennzeichen getrennt) (*.csv)|*.csv|TAB (Tabulatorzeichen ' +
      'getrennt) (*.tab)|*.tab'
    Left = 232
    Top = 10
  end
  object IB_Query1: TIB_Query
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'SELECT *'
      'FROM VERBRAUCHER'
      'FOR UPDATE')
    RequestLive = True
    Left = 792
    Top = 488
  end
  object IB_Query2: TIB_Query
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'SELECT *'
      'FROM AUFTRAG'
      'FOR UPDATE')
    RequestLive = True
    Left = 824
    Top = 488
  end
  object IB_Query3: TIB_Query
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'SELECT *'
      'FROM ZAEHLER'
      'FOR UPDATE')
    RequestLive = True
    Left = 856
    Top = 488
  end
  object Timer1: TTimer
    OnTimer = Timer1Timer
    Left = 888
    Top = 488
  end
  object IB_Query4: TIB_Query
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'SELECT  '
      ' ZAEHLER_NUMMER,'
      ' MATERIAL_NUMMER,'
      ' RID,'
      ' ART,'
      ' PLANQUADRAT,'
      ' KUNDE_STRASSE,'
      ' KUNDE_ORT,'
      ' STATUS,'
      ' KUNDE_ORTSTEIL'
      'FROM '
      ' AUFTRAG '
      'WHERE '
      ' (BAUSTELLE_R=:CROSSREF) AND'
      ' (RID=MASTER_R)'
      'ORDER BY'
      ' STRASSE')
    Left = 792
    Top = 520
  end
  object IB_Query5: TIB_Query
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'SELECT * '
      'FROM BAUSTELLE'
      'WHERE (RID=:CROSSREF)')
    Left = 824
    Top = 520
  end
  object IB_DSQL1: TIB_DSQL
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'UPDATE '
      ' AUFTRAG'
      'SET '
      ' MASTER_R=NULL'
      'WHERE '
      ' (RID_AT_IMPORT=:CROSSREF) ')
    Left = 920
    Top = 488
  end
  object IB_DSQL2: TIB_DSQL
    DatabaseName = '192.168.115.6:test.fdb'
    IB_Connection = DataModuleDatenbank.IB_Connection1
    SQL.Strings = (
      'DELETE FROM '
      ' AUFTRAG'
      'WHERE '
      ' (RID_AT_IMPORT=:CROSSREF)')
    Left = 952
    Top = 488
  end
end
