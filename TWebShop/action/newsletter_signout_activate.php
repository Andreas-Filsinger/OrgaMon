...
{ $tmp_person = new twebshop_person($person_r);
  ...
  { $mailings = new twebshop_mailings($tmp_person->getID());
	...
	{ $ids = array_diff($mailings->getActiveIDs(),array(TWEBSHOP_BILL_R_NEWSLETTER));
	  $mailings->updateInDataBase($ids);
	  $messagelist->add(SENTENCE_YOUR_EMAIL_ADDRESS_HAS_BEEN_REMOVED_FROM_NEWSLETTER);
	}
	else $messagelist->add(SENTENCE_YOUR_EMAIL_ADDRESS_HAS_BEEN_REMOVED_FROM_NEWSLETTER);
	unset($mailings);
  }
  else
...
else $errorlist->add(ERROR_INVALID_EMAIL);
