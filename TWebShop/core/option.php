...
class toption {
    ...
    const CLASS_NAME = "PHP5CLASS T_OPTION";
    ...
    public function setHTMLTemplate($template) {
        if (is_object($template) AND is_a($template, "ttemplate")) {
            $this->template = ($template->getTemplate(self::CLASS_NAME) != NULL) ? $template->getTemplate(self::CLASS_NAME) : $this->template;
        }
        else
            $this->template = $template;
    ...
    public function generateHTML() {
        if ($this->link_href != "" AND ($this->link_text != "" OR $this->icon_url != "")) {
            $html = "<a ";
            ...
            $this->html = "Option konnte nicht erzeugt werden !";
        return $this->html;
    }
