  ...
  try
    ...
    if PCSCReader.Protocol = prRaw then begin
      ...
      if PCSCResult = SCARD_S_SUCCESS then begin
        ...
        begin
        AddLogMemo('Card response data: ' + BufferToHexString(DataOut));
        AddLogMemo('Card response data: ' +BCDBufferToDecString(DataOut));
        end;
      end
...
try
 ...
 begin
  ...
  begin
   AddLogMemo('Error at record '+IntToStr(n));
   Break;
  end;
  AddLogMemo(IntToStr(n) + ' : ' + CardResponse + ' ('+IntToStr(length(CardResponse))+' bytes)');
  if length(CardResponse) =94 then
  begin
   GueltigBis := copy(CardResponse, 14, 2) + '.' + copy(CardResponse, 12, 2) + '.' + copy(CardResponse, 10, 2);
