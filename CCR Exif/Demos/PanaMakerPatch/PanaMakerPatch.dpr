...
uses
  ...
  SysUtils,
  Classes,
  XPMan, //need a manifest for Vista+ since we have 'Patch' in our file name...
...
begin
  WriteLn('-----------------------------------------------------------');
  WriteLn('                Panasonic MakerNote Patcher                ');
  WriteLn('-----------------------------------------------------------');
  WriteLn('');
  WriteLn('Usage:');
  ...
  WriteLn('  denote a MakerNote tag that already exists in the source');
...
begin
  ...
  try
    ...
    begin
      OutputProcessingError(JpegFile, 'Panasonic MakerNote not found');
      Exit;
    end;
    for ID in TagIDs do
...
begin
  try
    ...
    begin
      if ParamCount = 0 then
        MessageBox(0, 'This is a console application that requires command-line arguments' +
          ...
          'Panasonic MakerNote Patcher', MB_ICONINFORMATION)
      else
        OutputInfo;
      Exit;
    end;
    ...
    S := ParamStr(1);
    RetVal := FindFirst(S, {$WARN SYMBOL_PLATFORM OFF}faReadOnly or faArchive
                           {$WARN SYMBOL_PLATFORM ON}, SearchRec);
    if RetVal <> 0 then
    begin
