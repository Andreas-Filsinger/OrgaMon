...
function BCDBufferToDecString(Buffer: TBytes): string;
...
begin
  ...
  begin
    ...
    if (pos(s[1], cZiffern) > 0) then
      result := result + s[1];
    if (pos(s[2], cZiffern) > 0) then
      result := result + s[2];
  end;
  result := trim(result);
end;
...
begin
  case ErrorCode of
    ...
    SCARD_F_INTERNAL_ERROR:
      result := 'An internal consistency check failed.'; // 80100001;
    SCARD_F_UNKNOWN_ERROR:
      result := 'An internal error has been detected, but the source is unknown.';
      // 80100014;
    SCARD_F_WAITED_TOO_LONG:
      result := 'An internal consistency timer has expired.'; // 80100007;
    SCARD_E_CANCELLED:
      result := 'The action was cancelled by an SCardCancel request.';
      // 80100002;
    SCARD_E_INSUFFICIENT_BUFFER:
      result := 'The data buffer to receive returned data is too small for the returned data.';
      // 80100008;
    SCARD_E_INVALID_HANDLE:
      result := 'The supplied handle was invalid.'; // 80100003;
    ...
    SCARD_E_UNKNOWN_READER:
      result := 'The specified reader name is not recognized.'; // 80100009;
    SCARD_E_TIMEOUT:
      result := 'The user-specified timeout value has expired.'; // 8010000A;
    ...
    SCARD_E_UNKNOWN_CARD:
      result := 'The specified smart card name is not recognized.'; // 8010000D;
    ...
    SCARD_E_SYSTEM_CANCELLED:
      result := 'The action was cancelled by the system, presumably to log off or shut down.';
      // 80100012;
    SCARD_E_INVALID_ATR:
      result := 'An ATR obtained from the registry is not a valid ATR string.';
      // 80100015;
    SCARD_E_NOT_TRANSACTED:
      result := 'An attempt was made to end a non-existent transaction.';
      // 80100016;
    ...
    SCARD_E_PCI_TOO_SMALL:
      result := 'The PCI Receive buffer was too small.'; // 80100019;
    ...
    SCARD_E_CARD_UNSUPPORTED:
      ...
      // 8010001C;
    SCARD_E_NO_SERVICE:
      result := 'The Smart card resource manager is not running.'; // 8010001D;
    SCARD_E_SERVICE_STOPPED:
      result := 'The Smart card resource manager has shut down.'; // 8010001E;
    SCARD_E_UNEXPECTED:
      result := 'An unexpected card error has occurred.'; // 8010001F;
    ...
    SCARD_E_NO_FILE:
      ...
      // 80100026;
    SCARD_E_NO_ACCESS:
      result := 'Access is denied to this file.'; // 80100027;
    SCARD_E_WRITE_TOO_MANY:
      result := 'The smartcard does not have enough memory to store the information.';
      // 80100028;
    SCARD_E_BAD_SEEK:
      result := 'There was an error trying to set the smart card file object pointer.';
      // 80100029;
    SCARD_E_INVALID_CHV:
      result := 'The supplied PIN is incorrect.'; // 8010002A;
    SCARD_E_UNKNOWN_RES_MNG:
      result := 'An unrecognized error code was returned from a layered component.';
      // 8010002B;
    SCARD_E_NO_SUCH_CERTIFICATE:
      result := 'The requested certificate does not exist.'; // 8010002C;
    SCARD_E_CERTIFICATE_UNAVAILABLE:
      result := 'The requested certificate could not be obtained.'; // 8010002D;
    SCARD_E_NO_READERS_AVAILABLE:
      result := 'Cannot find a smart card reader.'; // 8010002E;
    ...
    SCARD_W_UNSUPPORTED_CARD:
      result := 'The reader cannot communicate with the card, due to ATR string configuration conflicts.';
      // 80100065;
    SCARD_W_UNRESPONSIVE_CARD:
      result := 'The smart card is not responding to a reset.'; // 80100066;
    ...
    SCARD_W_REMOVED_CARD:
      result := 'The smart card has been removed, so further communication is not possible.';
      // 80100069;
    ...
    SCARD_W_CHV_BLOCKED:
      ...
      // 8010006C;
    SCARD_W_EOF:
      result := 'The end of the smart card file has been reached.'; // 8010006D;
    SCARD_W_CANCELLED_BY_USER:
      result := 'The action was canceled by the user.'; // 8010006E;
    SCARD_W_CARD_NOT_AUTHENTICATED:
      result := 'No PIN was presented to the Smart card.'; // 8010006F;
  else
...
begin
  case ErrorCode of
    ...
    $9100:
      result := 'OK';
    $6200:
      result := 'State of non-volatile memory unchanged';
    $6281:
      result := 'State of non-volatile memory unchanged. Part of returned data may be corrupted.';
    $6282:
      result := 'State of non-volatile memory unchanged. End of file/record reached before reading Le bytes.';
    $6283:
      result := 'State of non-volatile memory unchanged. Selected file invalidated.';
    $6284:
      result := 'State of non-volatile memory unchanged. FCI not formatted.';

    $6300:
      result := 'State of non-volatile memory changed.';
    $6381:
      result := 'State of non-volatile memory changed. File filled up by the last write.';
    ...
    $6500:
      result := 'State of non-volatile memory changed.';
    $6581:
      result := 'State of non-volatile memory changed. Memory Failure.';

    $6700:
      result := 'Wrong length.';
    $6800:
      result := 'Functions in CLA not supported.';
    $6881:
      result := 'Functions in CLA not supported. Logical channel not supported.';
    $6882:
      result := 'Functions in CLA not supported. Secure messaging not supported.';

    $6900:
      result := 'Command not allowed. ';
    $6981:
      result := 'Command not allowed. Command incompatible with file structure.';
    $6982:
      result := 'Command not allowed. Security status not satisfied.';
    $6983:
      result := 'Command not allowed. Authentication method blocked.';
    $6984:
      result := 'Command not allowed. Referenced data invalidated.';
    $6985:
      result := 'Command not allowed. Conditions of use not satisfied.';
    $6986:
      result := 'Command not allowed. Command not allowed (no current EF).';
    $6987:
      result := 'Command not allowed. Expected SM data objects missing.';
    $6988:
      result := 'Command not allowed. SM data objects incorrect.';

    $6A00:
      result := 'Wrong parameters P1 or P2.';
    $6A80:
      result := 'Wrong parameter. Incorrect parameters in the data field.';
    $6A81:
      result := 'Wrong parameter. Function not supported.';
    $6A82:
      result := 'Wrong parameter. File not found.';
    $6A83:
      result := 'Wrong parameter. Record not found.';
    $6A84:
      result := 'Wrong parameter. Not enough memory space in the file.';
    $6A85:
      result := 'Wrong parameter. Lc inconsistent with TLV structure.';
    $6A86:
      result := 'Wrong parameters P1 or P2.';
    $6A87:
      result := 'Wrong parameter. Lc inconsistent with P1-P2.';
    $6A88:
      result := 'Wrong parameter. Referenced data not found.';

    $6B00:
      result := 'Wrong parameters P1 or P2.';
    $6D00:
      result := 'Instruction code not supported or invalid.';
    $6E00:
      result := 'Class byte not supported.';
    $6F00:
    ...
    begin
      case (ErrorCode shr 8) and $FF of
        $61:
          result := IntToStr(ErrorCode and $FF) +
            ' response bytes are still available.';
        $63:
          begin
            result := 'State of non-volatile memory changed.';
            if (ErrorCode and $F0) = $C0 then
              result := result + ' Counter = ' + IntToStr(ErrorCode and $0F);
          end;
        $64:
          result := 'State of non-volatile memory unchanged.';
        $6C:
          result := 'Wrong length Le: ' + IntToStr((ErrorCode and $FF)) +
            ' expected.';
        $90:
          result := 'OK. ' + IntToStr((ErrorCode and $FF)) +
            ' response byte(s) available';
        $91:
          result := 'DESFire specific error code: ' { + DESFireStatusToString(ErrorCode and $FF) };
        $9F:
          result := 'OK. ' + IntToStr((ErrorCode and $FF)) +
            ' response byte(s) available';
      else
